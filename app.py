...
def schedule_semester_non_electives(df_sem, holidays, base_date, exam_days, schedule_by_difficulty=False):
    ...
    for idx, row in df_sem.iterrows():
        if row['Exam Date'] == "" and row['IsCommon'] == "NO":
            priority = (row['Difficulty'] if schedule_by_difficulty and pd.notna(row['Difficulty']) else random.random(),
                        row['StudentCount'])
            heapq.heappush(subjects, (priority, idx, row['Branch'], row['Category']))

    # Group subjects by branch for compact scheduling
    branch_subjects = defaultdict(list)
    for _, idx, branch, category in subjects:
        branch_subjects[branch].append((idx, category))

    # Schedule subjects to minimize gaps
    current_date = base_date
    sem = df_sem["Semester"].iloc[0]
    time_slot = "10:00 AM - 1:00 PM" if (sem + 1) // 2 % 2 == 1 else "2:00 PM - 5:00 PM"

    for branch in branch_subjects:
        subjects_to_schedule = branch_subjects[branch]
        if schedule_by_difficulty:
            subjects_to_schedule.sort(key=lambda x: df_sem.loc[x[0], 'Difficulty'] or 0, reverse=True)
        ...
        for idx, _ in subjects_to_schedule:
            ...
            day_offset += 1  # Schedule on consecutive days within branch
...
def process_constraints(df, holidays, base_date, schedule_by_difficulty=False):
    ...
    for category in ['COMP', 'ELEC']:
        ...
        for module_code, group in common_subjects.groupby('ModuleCode'):
            ...
            df.loc[group.index, 'Time Slot'] = time_slot
            for branch in branches:
                exam_days[branch].add(exam_day.date())
                available_days[branch].add(exam_day.date())
            current_date = exam_day + timedelta(days=1)

    # Schedule non-common subjects and interleave electives
    final_list = []
    for sem in sorted(df["Semester"].unique()):
        ...
        if df_sem.empty:
            continue
        current_date = base_date
        scheduled_sem = schedule_semester_non_electives(df_sem, holidays, current_date, exam_days, schedule_by_difficulty)
    ...
    # Optimize gaps by reassigning exams
    def optimize_gaps(df_combined, max_gap=2):
        issues = []
        for branch in df_combined['Branch'].unique():
            ...
            for i in range(1, len(dates)):
                ...
                if gap > max_gap:
                    issues.append((branch, i, gap, dates[i-1], dates[i]))
        ...
        for branch, idx, gap, prev_date, curr_date in issues:
            ...
            if target_date.date() < curr_date.date() and all(target_date.date() not in exam_days[b] for b in [branch]):
                ...
                exam_days[branch].remove(curr_date.date())
    ...
    df_combined = optimize_gaps(df_combined)
...
def main():
    ...
    if 'total_span' not in st.session_state:
        st.session_state.total_span = 0
    ...
    if uploaded_file is not None:
        if st.button("🔄 Generate Timetable", type="primary", use_container_width=True):
            with st.spinner("Processing your timetable... Please wait..."):
                try:
                    holidays_set = set(h.date() for h in holiday_dates)
                    ...
                    if df_non_elec is not None and df_ele is not None:
                        ...
                        # Interleave electives within non-elective schedule
                        if not df_ele.empty:
                        ...
                        st.session_state.total_span = total_span
    ...
    if st.session_state.processing_complete:
        ...
        with col3:
            st.markdown(f'<div class="metric-card"><h3>🏫 {st.session_state.total_branches}</h3><p>Branches</p></div>',
                        unsafe_allow_html=True)
        with col4:
            st.markdown(f'<div class="metric-card"><h3>📅 {st.session_state.total_span}</h3><p>Days Span</p></div>',
                        unsafe_allow_html=True)
