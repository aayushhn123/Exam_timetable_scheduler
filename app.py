...
class TimetableOptimizer:
    ...
    def optimize_timetable(self, holidays=None):
        """Optimize timetable by filling empty slots and shift OE1, OE2, OE5 to last day"""
        if holidays:
        ...
        # Process each non-elective sheet
        for sheet_name, sheet_data in self.timetable_data.items():
            if sheet_data['is_elective']:
                continue  # Skip elective sheets for initial optimization
        ...
        # Find the last scheduled exam day after optimization
        all_dates = set()
        for sheet_data in self.timetable_data.values():
            df = sheet_data['data']
            for (date, time_slot), row in df.iterrows():
                try:
                    date_obj = pd.to_datetime(date, format="%d-%m-%Y")
        ...
        last_exam_day = max(all_dates) if all_dates else datetime.now().date()
        
        # Shift OE1, OE2, OE5 exams to the last exam day
        for sheet_name, sheet_data in self.timetable_data.items():
            if not sheet_data['is_elective']:
                continue  # Process only elective sheets
            
            df = sheet_data['data'].copy()
            oe_exams = defaultdict(list)
            
            # Collect all OE exams
            for (date, time_slot), row in df.iterrows():
                for col in df.columns:
                    exam = row[col]
                    if pd.isna(exam) or str(exam).strip() == "---":
                        continue
                    # Extract OE type (assuming format like "Subject [OE1]")
                    oe_match = re.search(r'\[OE(\d+)\]', str(exam))
                    if oe_match:
                        oe_type = oe_match.group(1)
                        if oe_type in ['1', '2', '5']:
                            oe_exams[oe_type].append((date, time_slot, col, exam))
            
            # Assign OE1, OE2, OE5 to last exam day with appropriate time slots
            time_slots = ["10:00 AM - 1:00 PM", "2:00 PM - 5:00 PM"]
            slot_index = 0
            for oe_type, exams in oe_exams.items():
                for date, time_slot, col, exam in exams:
                    new_time_slot = time_slots[slot_index % len(time_slots)]
                    slot_index += 1
                    df.loc[(date, time_slot), col] = "---"
                    df.loc[(last_exam_day.strftime("%d-%m-%Y"), new_time_slot), col] = exam
                    self.optimization_log.append(
                        f"Shifted {exam} from {date} to {last_exam_day.strftime('%d-%m-%Y')} ({new_time_slot}) for OE{oe_type}"
                    )
...
def main():
    ...
    - Shift OE1, OE2, OE5 to the last scheduled exam day
